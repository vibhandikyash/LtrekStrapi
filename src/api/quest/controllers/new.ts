export default ({ strapi }) => ({
  async index(ctx) {
    ...
    const query = await strapi.db.connection.raw(
      `WITH lonQuery AS (${lonQuery}),latQuery AS (${latQuery}) SELECT * FROM quests t1 INNER JOIN ST_DistanceSphere(ST_MakePoint((SELECT longitude FROM lonQuery WHERE lonQuery.quest_id = t1.id),(SELECT latitude FROM latQuery WHERE latQuery.quest_id = t1.id)),ST_MakePoint(${lng},${lat})) AS distance ON ST_DistanceSphere(ST_MakePoint((SELECT longitude FROM lonQuery WHERE lonQuery.quest_id = t1.id),(SELECT latitude FROM latQuery WHERE latQuery.quest_id = t1.id)),
